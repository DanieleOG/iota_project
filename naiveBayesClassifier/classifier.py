...
class Classifier(object):
    ...
    def classify(self, text):
        ...
        for className in classes:
            ...
            # we are calculating the probablity of seeing each token in the text of this class
            # P(Token_1|Class_i)
            tokensProbs = [self.getTokenProb(token, className) for token in tokens]
            
            # calculating the probablity of seeing the the set of tokens in the text of this class
    ...
    def getTokenProb(self, token, className):
        ...
        # if the token is not seen in the training set, so not indexed, then we return None not to include it into calculations.
        try:
